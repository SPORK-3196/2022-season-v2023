...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    X1J_B.toggleOnTrue(new PlayMusic(Drivetrain));
    // whenHeld(new PlayMusic(Drivetrain), false);
    // .whenActive(new PlayMusic(Drivetrain));

    X1J_A.whileTrue(new TargetOrientation(Drivetrain)).whileTrue(new VisionTargetShooting(Lighting, primaryYaw));
    X1J_X.whileTrue(new CargoOrientation(Drivetrain)).whileTrue(new VisionTargetShooting(Lighting, backupYaw));

    // X2J_X.whenHeld(new IntakeCargo(Intake)).whenHeld(new IntakeLighting(Lighting));

    X2J_X.whileTrue(new IntakeActivation(Climber, Intake)).whileTrue(new IntakeLighting(Lighting));
    X2J_X.onFalse(new RaiseArms(Climber));

    X2J_B.whileTrue(new OuttakeCargo(Intake, Index)).whileTrue(new IntakeLighting(Lighting));
    X2J_A.whileTrue(new AutoShoot(Shooter)).whileTrue(new IndexShootingUpper(Index)).whileTrue(new VisionTargetShooting(Lighting, primaryYaw));
    // X2J_A.whenHeld(new TweenShoot(Shooter)).whenHeld(new IndexShooting(Index)).whenHeld(new ShootLighting(Lighting));
    X2J_Y.whileTrue(new ToggleArms(Climber)).onTrue(new ToggleClimberLighting(Lighting));

    X2J_RB.whileTrue(new ExtendClimber(Climber, 0.45)).whileTrue(new ExtendClimbLighting(Lighting));
    X2J_LB.whileTrue(new RetractClimber(Climber, 0.45)).whileTrue(new RetractClimbLighting(Lighting));

    X2J_LS.whileTrue(new BabyShoot(Shooter)).whileTrue(new IndexShootingLower(Index)).whileTrue(new ShootLighting(Lighting));
    
    X2J_RS.whileTrue(new TweenShoot(Shooter)).whileTrue(new IndexShootingLower(Index)).whileTrue(new ShootLighting(Lighting));
